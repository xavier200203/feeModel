const {
    attachLNBridge, readConfig, sleep, attachEBTC
...
const main = async () => {
    ...
    let withdrawAddress = await readConfig("config", "LNBRIDGE");
    let contract = await attachLNBridge(owner, withdrawAddress);
    console.log("attachLNBridge address:", contract.address);
    ...
    let sig = await web3.eth.sign(pendingID, accounts[0].address);
    ...
    owner = accounts[1];
    let tokenAddress = await readConfig("config", "EBTC");
    let wbtc = await attachEBTC(owner, tokenAddress);
